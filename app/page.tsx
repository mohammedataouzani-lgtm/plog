      ...
      <div className="grid md:grid-cols-[1fr_1.4fr] gap-8 items-start mb-16">
        ...
        <div className="border border-border rounded-lg p-6">
          <div className="flex items-center justify-between mb-3">
            <span className="font-display text-lg font-light">Scrutins récents</span>
          ...
          <div className="border-t border-border">
            {(stats.recentScrutins as any[]).map((s: any) => (
              <Link key={s.uid} href={`/scrutins/${s.uid}`} className="flex items-start gap-3 border-b border-border py-3 hover:bg-card group">
                <span className={`text-xs font-semibold mt-0.5 w-4 shrink-0 ${s.sort === "adopté" ? "text-emerald-600" : "text-red-500"}`}>{s.sort === "adopté" ? "✓" : "✗"}</span>
